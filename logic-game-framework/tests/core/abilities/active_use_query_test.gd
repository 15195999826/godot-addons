...
##  - Ability 级短路与真实激活（deliver_to_ability → Ability.receive_event）同判（disabled / expired）
...
func _init() -> void:
	...
	TestFramework.register_test("can_activate mirrors real-activation short-circuits", _test_ability_level_shortcircuits)
...
func _test_denied_by_condition() -> void:
	...
	ability_set.get_owner_instance().event_processor.deliver_to_ability(activate_dict, ability.owner_actor_id, ability.id)
...
func _test_ability_level_shortcircuits() -> void:
	...
	# disabled：Ability.receive_event 顶层短路，查询同判
