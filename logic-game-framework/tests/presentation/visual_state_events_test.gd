...
## VisualState 账本半边（录像台面重建 + 事件直改 + 只读视图）的合同钉子
##
## 钉住合同：
## - initialize_from_replay 按 world_snapshot 建每个 actor（逻辑坐标取 position 前两分量 / hp / max_hp / facing / 队伍），
##   空 id 跳过，建完对每个 actor 广播一次 actor_state_changed；get_actors_snapshot 是深拷贝
## - actor_spawned 事件直改（VisualUpdater.apply_event）：新 actor 入账并广播 actor_spawned + actor_state_changed；
##   同 id 重复 spawn 忽略；actor 缺 id 用事件的 actor_id；空 actor dict 忽略
...
var _updater := VisualUpdater.new()


func _init() -> void:
	TestFramework.register_test("VisualState initialize_from_replay 建台面、跳空 id、每人广播一次、快照深拷贝", _test_initialize_from_replay)
	TestFramework.register_test("VisualState actor_spawned 事件直改入账并广播，重复 spawn 忽略", _test_spawn_event)
	TestFramework.register_test("VisualState actor_destroyed 事件直改归零，actor_died 只发一次", _test_destroy_event)
...
func _test_initialize_from_replay() -> void:
	...
	TestFramework.assert_true(u1.position.is_equal_approx(Vector2(2.0, -1.0)))
	...
	var u2: ActorVisualState = snapshot["u2"]
	TestFramework.assert_equal(1, u2.team)
	# 没有 facing 属性默认 0
	TestFramework.assert_equal(0, u2.facing_direction)
	TestFramework.assert_equal(0, world.get_time_ms())

	# 快照是深拷贝：改拷贝不影响账本
	u1.target_hp = 1.0
	u1.position = Vector2(9.0, 9.0)
	TestFramework.assert_near(_actor(world, "u1").target_hp, 40.0)
	TestFramework.assert_true(_actor(world, "u1").position.is_equal_approx(Vector2(2.0, -1.0)))
...
func _test_spawn_event() -> void:
	...
	_updater.apply_event(world, _spawn_event("t1", totem))
	TestFramework.assert_equal("t1", ",".join(spawned))
	TestFramework.assert_equal("t1", ",".join(changed))
	var t1 := _actor(world, "t1")
	TestFramework.assert_true(t1.position.is_equal_approx(Vector2(3.0, 4.0)))
	...
	TestFramework.assert_equal(2, world.as_query().get_all_actor_ids().size())
	...
	_updater.apply_event(world, _spawn_event("t2", {"type": PROBE_TYPE, "position": [0, 0, 0], "attributes": {}}))
	...
	_updater.apply_event(world, _spawn_event("t3", {}))
	_updater.apply_event(world, {"kind": GameEvent.ACTOR_SPAWNED_EVENT, "actor_id": "t4"})
...
func _test_destroy_event() -> void:
	...
	_updater.apply_event(world, {"kind": GameEvent.ACTOR_DESTROYED_EVENT, "actor_id": "u1"})
	...
	_updater.apply_event(world, {"kind": GameEvent.ACTOR_DESTROYED_EVENT, "actor_id": "ghost"})
	_updater.apply_event(world, {"kind": GameEvent.ACTOR_DESTROYED_EVENT})
	TestFramework.assert_equal(1, died.size())
...
func _test_max_hp_change_clamps() -> void:
	...
	_updater.apply_event(world, _max_hp_event("u1", 60.0))
	var u1 := _actor(world, "u1")
	...
	# 上限回升不回血
	_updater.apply_event(world, _max_hp_event("u1", 120.0))
	u1 = _actor(world, "u1")
	...
	# 别的属性不管
	_updater.apply_event(world, _max_hp_event("u1", 5.0, "atk"))
	...
	_updater.apply_event(world, _max_hp_event("u1", -5.0))
	u1 = _actor(world, "u1")
	...
	# 未知 actor：忽略
	_updater.apply_event(world, _max_hp_event("ghost", 10.0))
...
func _test_reset_to_restores() -> void:
	...
	# 打乱：扣血致死、spawn 新人、推进时间、改位置、挂一条一次性效果
	var kill := ActionStepper.ActiveAction.new("k", VisualHpDeltaAction.new("u1", -100.0))
	kill.progress = 1.0
	kill.is_delaying = false
	var batch: Array[ActionStepper.ActiveAction] = [kill]
	_updater.apply_actions(world, batch)
	_updater.apply_event(world, _spawn_event("t1", {"type": PROBE_TYPE, "position": [0, 0, 0], "attributes": {}}))
	world.advance_time(1234)
	world.set_actor_position("u1", Vector2(5.0, 5.0))
	var text := VisualEffectPayload.FloatingText.new()
	text.id = "ft"
	text.duration = 99999.0
	world.spawn_effect(VisualAction.KIND_FLOATING_TEXT, text)
	TestFramework.assert_equal("u1", ",".join(died))
	TestFramework.assert_equal(2, world.get_actors_snapshot().size())
	TestFramework.assert_true(world.has_effect(VisualAction.KIND_FLOATING_TEXT, "ft"))
	...
	TestFramework.assert_true(u1.position.is_equal_approx(Vector2(1.0, 1.0)))
	# 插值位置一并重置
	TestFramework.assert_true(world.get_actor_position("u1").is_equal_approx(Vector2(1.0, 1.0)))
	TestFramework.assert_equal(0, world.get_time_ms())
	# 一次性效果清空
	TestFramework.assert_false(world.has_effect(VisualAction.KIND_FLOATING_TEXT, "ft"))
...
func _test_query_reads() -> void:
	...
	world.initialize_from_replay(_record(actors))
	var query := world.as_query()

	TestFramework.assert_near(query.get_actor_hp("u1"), 40.0)
	TestFramework.assert_near(query.get_actor_max_hp("u1"), 100.0)
	TestFramework.assert_true(query.is_actor_alive("u1"))
	TestFramework.assert_true(query.is_actor_alive("u2"), "开局 hp 0 也按存活入账，死活由事件决定")
	TestFramework.assert_equal(1, query.get_actor_team("u1"))
	TestFramework.assert_equal("右一", query.get_actor_display_name("u1"))
	TestFramework.assert_equal("u1,u2", ",".join(query.get_all_actor_ids()))
	# 逻辑平面坐标：账本与只读视图同一口径
	TestFramework.assert_true(query.get_actor_position("u1").is_equal_approx(Vector2(2.0, -1.0)))
	TestFramework.assert_true(world.get_actor_position("u1").is_equal_approx(Vector2(2.0, -1.0)))
	TestFramework.assert_true(query.has_actor("u1"))
	TestFramework.assert_true(query.get_animation_config() != null)

	# 未知 actor 默认值
	TestFramework.assert_near(query.get_actor_hp("ghost"), 0.0)
	TestFramework.assert_near(query.get_actor_max_hp("ghost"), 100.0)
	TestFramework.assert_false(query.is_actor_alive("ghost"))
	TestFramework.assert_equal(0, query.get_actor_team("ghost"))
	TestFramework.assert_equal("", query.get_actor_display_name("ghost"))
	TestFramework.assert_false(query.has_actor("ghost"))
	TestFramework.assert_true(query.get_actor_position("ghost").is_equal_approx(Vector2.ZERO))
