...
func _test_projectile() -> void:
	...
	TestFramework.assert_equal(VisualProjectileAction.ProjectileType.ARROW, card.projectile_type)

	# 快弹夹到 300 ms 下限
	var fast := ProjectileEvents.create_projectile_launched_event("p2", "u1", Vector3(0, 0, 0), "fireball", 200.0, "u2", Vector3(3, -1, 0))
	var fast_card := translator.translate(fast, world.as_query())[0] as VisualProjectileAction
	TestFramework.assert_near(fast_card.duration, 300.0)
	TestFramework.assert_equal(VisualProjectileAction.ProjectileType.FIREBALL, fast_card.projectile_type)
	...
	var ghost_card := translator.translate(ghost, world.as_query())[0] as VisualProjectileAction
	...
	var active := ActionStepper.ActiveAction.new("m", move)
	active.progress = 0.5
	active.is_delaying = false
	var batch: Array[ActionStepper.ActiveAction] = [active]
	VisualUpdater.new().apply_actions(world, batch)
	var flying := translator.translate(event, world.as_query())[0] as VisualProjectileAction
	TestFramework.assert_true(flying.start_position.is_equal_approx(Vector2(1.0, 0.0)))


func _test_push_blocked() -> void:
	var translator := FrontendPushBlockedTranslator.new()
	var query := _world([{"id": "u1"}]).as_query()
	var event := BattleEvents.PushBlockedEvent.create("u1", {"q": 0, "r": 0}, {"q": 1, "r": -1}, "edge", "", "u2").to_dict()
	var actions := translator.translate(event, query)
	TestFramework.assert_equal(1, actions.size())
	var bump := actions[0] as VisualBumpAction
	...
	TestFramework.assert_equal(0, translator.translate(same, query).size())


func _test_cone_overlay() -> void:
	var translator := FrontendStageCueTranslator.new()
	var query := _world([{"id": "u1"}]).as_query()
	...
	var actions := translator.translate(grid_event, query)
	TestFramework.assert_equal(1, actions.size())
	var overlay := actions[0] as FrontendConeDebugOverlayAction
	TestFramework.assert_equal(FrontendConeDebugOverlayAction.KIND, overlay.kind)
	# 去重
	...
	TestFramework.assert_true(overlay.fill_color.is_equal_approx(FrontendStageCueTranslator.CONE_DEBUG_FILL_COLOR_GRID))
	...
	var angle_overlay := translator.translate(angle_event, query)[0] as FrontendConeDebugOverlayAction
	...
	TestFramework.assert_true(angle_overlay.fill_color.is_equal_approx(FrontendStageCueTranslator.CONE_DEBUG_FILL_COLOR_ANGLE))

	# hex 私有卡片的记账：挂进更新器后按卡片 id 入账一次，effect_spawned 带 Payload
	var world := _world([{"id": "u1"}])
	var updater := VisualUpdater.new()
	updater.register_handler(FrontendConeDebugOverlayAction.KIND, FrontendConeDebugOverlayAction.apply)
	var spawned: Array[String] = []
	world.effect_spawned.connect(func(kind: StringName, payload: VisualEffectPayload.Effect) -> void:
		spawned.append("%s:%s:%d" % [kind, payload.id, (payload as FrontendConeDebugOverlayAction.Payload).cells.size()]))
	var active := ActionStepper.ActiveAction.new("c1", overlay)
	active.progress = 0.1
	active.is_delaying = false
	var batch: Array[ActionStepper.ActiveAction] = [active]
	...
	active.progress = 0.6
	updater.apply_actions(world, batch)
	TestFramework.assert_equal("cone_debug_overlay:c1:2", ",".join(spawned))


func _test_control_floating_text() -> void:
	var translator := FrontendStageCueTranslator.new()
	var query := _world([{"id": "u1"}, {"id": "u2", "q": 2, "r": -1}]).as_query()
	var targets: Array[String] = ["u2"]
	var event := GameEvent.StageCue.create("u1", targets, HexBattleCues.CONTROL_STUNNED).to_dict()
	var actions := translator.translate(event, query)
	TestFramework.assert_equal(1, actions.size())
	var text := actions[0] as VisualFloatingTextAction
