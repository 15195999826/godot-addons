...
static func _compute_footprint_cells(center: HexCoord, footprint_size: Vector2i) -> Array:
	return compute_footprint_cells_core(center, footprint_size.x, footprint_size.y)


## 从 RtsObstructionShapeStatic 算 footprint cells (新算法; 用 obstruction_shape.center
## 当中心, w/h ÷ cell_size 推 cells_w/h). 跟 RtsBuildingActor.get_footprint_cells 新路径
## 走同一 core helper, 无双份算法漂移.
##
## 用于:
##   - placement 校验时
##   - frontend ghost preview 渲染
##   - actor.get_footprint_cells delegate
static func compute_footprint_cells_from_shape(shape: RtsObstructionShapeStatic, grid: RtsBattleGrid) -> Array:
	if shape == null or grid == null:
		return []
	var center: HexCoord = grid.world_to_coord(shape.center)
	# int(round(...)) 显式整数化, 避免浮点精度漂移 (与 actor.get_footprint_cells 一致)
	var cells_w: int = int(round(shape.width / grid.cell_size))
	var cells_h: int = int(round(shape.height / grid.cell_size))
	return compute_footprint_cells_core(center, cells_w, cells_h)


## Core AABB cells 算法, 偶数尺寸"左上偏置" (上半左半), 奇数居中.
## actor.get_footprint_cells / _compute_footprint_cells / compute_footprint_cells_from_shape
## 共享, 严格保留 codex P1 #1 锚定的偏置方向.
static func compute_footprint_cells_core(center: HexCoord, cells_w: int, cells_h: int) -> Array:
