...
func compute_short_path_immediate(req: RtsShortPathRequest, obstr_mgr: RtsObstructionManager) -> RtsWaypointPath:
	...
	if req.start.distance_squared_to(req.goal.center) < 1.0:
		var same := RtsWaypointPath.new()
		same.push_back(req.goal.center)
		return same

	var bounds: Rect2 = _compute_search_bounds(req.start, req.goal.center, req.range_px)
	...
	# 顺序契约 (§12.3): start, goal, OBB corners (statics 升序 × get_corners() 0..3),search bounds TL/TR/BL/BR
	var vertices: Array[Vector2] = []
	vertices.append(req.start)
	var start_idx: int = 0
	vertices.append(req.goal.center)
	var goal_idx: int = 1
...
##
## **找不到路径**:open 耗尽未达 goal_idx → 返空 RtsWaypointPath(M6a 不做 best-so-far)。
func _astar_lazy_visibility(
	...
	# 找不到 = 空 path (M6a;M6b 改 best-so-far)
