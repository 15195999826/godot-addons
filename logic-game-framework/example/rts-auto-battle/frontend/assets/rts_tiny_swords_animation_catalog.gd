class_name RtsTinySwordsAnimationCatalog


const ROOT: String = "res://addons/logic-game-framework/example/rts-auto-battle/frontend/assets/tiny_swords"
...
static func create_frames(sequence: Dictionary) -> SpriteFrames:
	...
	Log.assert_crash(texture != null, "RtsTinySwordsAnimationCatalog", "Failed to load texture: %s" % path)
...
static func _load_manifest_entries() -> Array[Dictionary]:
	...
	Log.assert_crash(file != null, "RtsTinySwordsAnimationCatalog", "Failed to open manifest: %s" % MANIFEST_PATH)
	var content := file.get_as_text()
	var parsed: Variant = JSON.parse_string(content)
	Log.assert_crash(parsed is Dictionary, "RtsTinySwordsAnimationCatalog", "Invalid manifest JSON")
...
static func _load_library() -> Dictionary:
	...
	Log.assert_crash(file != null, "RtsTinySwordsAnimationCatalog", "Failed to open library: %s" % LIBRARY_PATH)
	var parsed: Variant = JSON.parse_string(file.get_as_text())
	Log.assert_crash(parsed is Dictionary, "RtsTinySwordsAnimationCatalog", "Invalid animation library JSON")
