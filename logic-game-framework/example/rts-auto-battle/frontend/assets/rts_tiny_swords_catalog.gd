class_name RtsTinySwordsCatalog
...
const _ROOT: String = "res://addons/logic-game-framework/example/rts-auto-battle/frontend/assets/tiny_swords"
...
static func create_unit_frames(unit_kind: String, team_id: int) -> SpriteFrames:
	Log.assert_crash(_UNIT_PATHS.has(unit_kind), "RtsTinySwordsCatalog", "Unknown unit kind: %s" % unit_kind)
	var by_team: Dictionary = _UNIT_PATHS[unit_kind] as Dictionary
	Log.assert_crash(by_team.has(team_id), "RtsTinySwordsCatalog", "Unknown team id: %d" % team_id)
...
static func load_building_texture(building_kind: String, team_id: int) -> Texture2D:
	Log.assert_crash(_BUILDING_PATHS.has(building_kind), "RtsTinySwordsCatalog", "Unknown building kind: %s" % building_kind)
...
static func load_resource_texture(resource_kind: String) -> Texture2D:
	Log.assert_crash(_RESOURCE_PATHS.has(resource_kind), "RtsTinySwordsCatalog", "Unknown resource kind: %s" % resource_kind)
	return _load_texture(_RESOURCE_PATHS[resource_kind] as String)


static func load_decor_texture(decor_kind: String) -> Texture2D:
	Log.assert_crash(_DECOR_PATHS.has(decor_kind), "RtsTinySwordsCatalog", "Unknown decor kind: %s" % decor_kind)
...
static func load_effect_texture(effect_kind: String, team_id: int = TEAM_BLUE) -> Texture2D:
	team_id = team_id
	Log.assert_crash(_EFFECT_PATHS.has(effect_kind), "RtsTinySwordsCatalog", "Unknown effect kind: %s" % effect_kind)
...
static func _add_sheet_animation(
	...
	Log.assert_crash(column_count >= frame_count, "RtsTinySwordsCatalog", "Not enough frames in sheet row: %s" % path)
	Log.assert_crash(row_count > row, "RtsTinySwordsCatalog", "Missing sheet row: %s" % path)
...
static func _load_texture(path: String) -> Texture2D:
	...
	Log.assert_crash(texture != null, "RtsTinySwordsCatalog", "Failed to load texture: %s" % path)
	return texture
