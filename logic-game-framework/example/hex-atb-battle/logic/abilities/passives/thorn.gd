...
## 攻击根本没打到肉）。on-damage-taken 反应默认看 actual_life_damage > 0（护盾结算与扣血顺序见
## README「设计铁律」与 hex_battle_shield_resolver.gd 头注释）。新增其它 on-damage-taken 反应（吸血、受击 buff）时
## 必须沿用此 filter 模板。
static func _thorn_filter() -> Callable:
	return func(event_dict: Dictionary, ctx: AbilityLifecycleContext) -> bool:
		...
		var has_life_damage := damage_event.actual_life_damage > 0.0
		return is_target and has_source and not_self_damage and not damage_event.is_reflected and has_life_damage
