## ProjectileTranslator - 投射物事件翻译员
##
## 将投射物相关事件翻译为卡片：
## - projectile_launched: 创建投射物飞行动画
## - projectile_hit: 命中特效
## - projectile_miss: 消散特效
class_name FrontendProjectileTranslator
extends Translator


func _init() -> void:
	translator_name = "ProjectileTranslator"
...
func translate(event: Dictionary, query: VisualStateQuery) -> Array[VisualAction]:
	...
	match kind:
		ProjectileEvents.PROJECTILE_LAUNCHED_EVENT:
			return _translate_launched(event, query)
		ProjectileEvents.PROJECTILE_HIT_EVENT:
			return _translate_hit(event, query)
		ProjectileEvents.PROJECTILE_MISS_EVENT:
			return _translate_miss(event, query)
		_:
			return []


## 翻译投射物发射事件
func _translate_launched(event: Dictionary, query: VisualStateQuery) -> Array[VisualAction]:
	...
	var start_position := _resolve_position(event, "start_position", source_actor_id, query)
	var target_position := _resolve_position(event, "target_position", target_actor_id, query)

	# 飞行时间 = 逻辑平面距离 / 速度(hex 逻辑层的投射物本来就在 (q, r) 平面上飞,speed 也是这个平面的);
	# 最小 300ms,确保投射物可见
	var raw_duration := VisualProjectileAction.calculate_duration(start_position, target_position, speed)
	...
	# 创建投射物飞行卡片
	var projectile_action := VisualProjectileAction.new(
		projectile_id,
		source_actor_id,
		start_position,
		target_position,
...
## 翻译投射物命中事件
func _translate_hit(event: Dictionary, query: VisualStateQuery) -> Array[VisualAction]:
	var config := query.get_animation_config()
	...
	# 命中闪白特效
	if target_actor_id != "":
		var hit_flash := VisualProceduralVfxAction.new(
...
## 翻译投射物未命中事件
func _translate_miss(_event: Dictionary, _query: VisualStateQuery) -> Array[VisualAction]:
	# 未命中时可以添加消散特效，暂时不做处理
	return []


## 位置优先取账本(actor 的在飞插值),账本没有这个 actor 再读事件字段(逻辑层给的逻辑平面坐标)
func _resolve_position(event: Dictionary, field: String, actor_id: String, query: VisualStateQuery) -> Vector2:
	if actor_id != "" and query.has_actor(actor_id):
		return query.get_actor_position(actor_id)
...
## 解析投射物类型字符串
func _parse_projectile_type(type_str: String) -> VisualProjectileAction.ProjectileType:
	match type_str.to_lower():
		"arrow":
			return VisualProjectileAction.ProjectileType.ARROW
		"fireball":
			return VisualProjectileAction.ProjectileType.FIREBALL
		_:
			return VisualProjectileAction.ProjectileType.ENERGY
