## BuffTranslator - buff 状态变化的事件翻译员
##
## 订阅 4 类事件,翻译为 VisualBuffStateAction:
...
class_name FrontendBuffTranslator
extends Translator
...
const BUFF_REGISTRY := {
	...
	HexBattleSilenceBuff.CONFIG_ID: {
		"short": "🤐",
...
func _init() -> void:
	translator_name = "BuffTranslator"
...
func _handle_granted(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	actions.append(VisualBuffStateAction.new(
		actor_id, VisualBuffStateAction.Op.ADD, ability_id, summary
	))


func _handle_stacks_changed(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	actions.append(VisualBuffStateAction.new(
		actor_id, VisualBuffStateAction.Op.UPDATE, ability_id, summary
	))
...
func _handle_removed(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	if actor_id.is_empty() or ability_id.is_empty():
		return
	actions.append(VisualBuffStateAction.new(
		actor_id, VisualBuffStateAction.Op.REMOVE, ability_id, null
	))
