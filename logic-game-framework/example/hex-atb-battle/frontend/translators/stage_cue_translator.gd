## StageCueTranslator - 舞台提示事件翻译员
##
## 将 stage_cue 事件翻译为视觉特效卡片
## 根据 cue_id 决定播放什么特效：
## - melee_slash, melee_heavy, melee_combo → 播放攻击箭头特效
## - magic_fireball, ranged_arrow → 不播放（有投射物动画）
## - magic_heal → 治疗光束特效
class_name FrontendStageCueTranslator
extends Translator
...
## 复用 VisualFloatingTextAction (已存在的飘字系统), 不为单技能搭独立 VFX。
## 加新 control 飘字 = 多写一行配置即可。
const CONTROL_FLOATING_TEXTS := {
	HexBattleCues.CONTROL_STUNNED: {
		"text": "眩晕!",
		"color": Color(0.95, 0.85, 0.2),  # 金黄, 与 BuffTranslator Stun entry 同色
		"style": VisualFloatingTextAction.FloatingTextStyle.CRITICAL,
	},
	HexBattleCues.CONTROL_SILENCED: {
		"text": "沉默!",
		"color": Color(0.55, 0.35, 0.85),  # 紫色, 与 BuffTranslator Silence entry 同色
		"style": VisualFloatingTextAction.FloatingTextStyle.CRITICAL,
	},
	HexBattleCues.CONTROL_BROKEN: {
		"text": "破坏!",
		"color": Color(0.45, 0.45, 0.55),  # 暗紫灰, 与 BuffTranslator Break entry 同色
		"style": VisualFloatingTextAction.FloatingTextStyle.CRITICAL,
	},
	...
	HexBattleCues.CONTROL_CLEANSED: {
		...
		"style": VisualFloatingTextAction.FloatingTextStyle.HEAL,
	},
	HexBattleCues.SWAP_BLINK: {
		"text": "换位!",
		"color": Color(0.6, 0.85, 0.95),  # 浅青蓝, 瞬移感
		"style": VisualFloatingTextAction.FloatingTextStyle.NORMAL,
	},
...
func _init() -> void:
	translator_name = "StageCueTranslator"
...
func translate(event: Dictionary, query: VisualStateQuery) -> Array[VisualAction]:
	...
	if cue_id in MELEE_ATTACK_CUES:
		actions.append_array(_create_melee_attack_vfx(source_id, target_ids, cue_id, query))
	# 治疗类 cue → 治疗光束特效
	elif cue_id in HEAL_CUES:
		actions.append_array(_create_heal_vfx(source_id, target_ids, query))
	# 斩杀击杀 → 猩红冲击波(放大)醒目特效,区别于普通近战挥击
	elif cue_id == EXECUTE_KILL_CUE:
		actions.append_array(_create_execute_kill_vfx(source_id, target_ids, query))
	# 控制状态 cue (Stun / Silence / Break) → 目标头顶飘字 (复用 floating-text pattern)
	elif cue_id in CONTROL_FLOATING_TEXTS:
		actions.append_array(_create_control_floating_text(target_ids, cue_id, query))
	# Phase E: cone debug overlay → checked cells fill + geometry boundary
	elif cue_id in CONE_DEBUG_CUES:
		actions.append_array(_create_cone_overlay(cue_id, params, query))
...
func _create_control_floating_text(
	...
	var anim_cfg := query.get_animation_config()
	for target_id in target_ids:
		var pos := query.get_actor_position(target_id)
		actions.append(VisualFloatingTextAction.new(
			target_id,
			cfg.get("text", "") as String,
			cfg.get("color", Color.WHITE) as Color,
			pos,
			cfg.get("style", VisualFloatingTextAction.FloatingTextStyle.NORMAL) as int,
...
func _create_melee_attack_vfx(
	...
	var source_position := query.get_actor_position(source_id)
	var source_team := query.get_actor_team(source_id)

	# 为每个目标创建攻击特效
	for target_id in target_ids:
		var target_position := query.get_actor_position(target_id)

		var attack_vfx := VisualAttackVfxAction.new(
			source_id,
			target_id,
			source_position,
			target_position,
...
## 根据 cue_id 获取特效类型
func _get_vfx_type_for_cue(cue_id: String) -> VisualAttackVfxAction.AttackVFXType:
	match cue_id:
		"melee_slash":
			return VisualAttackVfxAction.AttackVFXType.SLASH
		"melee_heavy":
			return VisualAttackVfxAction.AttackVFXType.IMPACT
		"melee_combo":
			return VisualAttackVfxAction.AttackVFXType.THRUST
		"totem_attack":
			return VisualAttackVfxAction.AttackVFXType.IMPACT
		_:
			return VisualAttackVfxAction.AttackVFXType.SLASH
...
func _create_heal_vfx(
	...
	for target_id in target_ids:
		...
		var heal_vfx := VisualAttackVfxAction.new(
			source_id,
			target_id,
			source_position,
			target_position,
			config.attack_vfx_duration,
			VisualAttackVfxAction.AttackVFXType.SLASH,
...
func _create_execute_kill_vfx(
	...
	for target_id in target_ids:
		...
		var kill_vfx := VisualAttackVfxAction.new(
			source_id,
			target_id,
			source_position,
			target_position,
			EXECUTE_KILL_VFX_DURATION,
			VisualAttackVfxAction.AttackVFXType.IMPACT,
		...
		var callout := VisualFloatingTextAction.new(
			target_id,
			"斩杀!",
			Color(1.0, 0.15, 0.1),
			target_position,
			VisualFloatingTextAction.FloatingTextStyle.CRITICAL,
			query.get_animation_config().damage_floating_text_duration,
