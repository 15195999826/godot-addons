## ShieldBarTranslator - 护盾状态变化的事件翻译员
##
## 与 BuffTranslator 平行的视觉管线,专门维护 actor.shields 数组(血条上方
## 独立护盾条的数据来源)。
##
## 订阅 3 类事件,翻译为 VisualShieldStateAction:
...
class_name FrontendShieldBarTranslator
extends Translator
...
func _init() -> void:
	translator_name = "ShieldBarTranslator"
...
func _handle_granted(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	var data := ShieldSummary.find_shield_component_data(payload)
	if data.is_empty():
		return
	...
	actions.append(VisualShieldStateAction.new(
		actor_id, VisualShieldStateAction.Op.ADD, ability_id, summary
	))


func _handle_removed(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	if actor_id.is_empty() or ability_id.is_empty():
		return
	actions.append(VisualShieldStateAction.new(
		actor_id, VisualShieldStateAction.Op.REMOVE, ability_id, null
	))


func _handle_damage(event: Dictionary, actions: Array[VisualAction]) -> void:
	...
	for record_variant in consumption:
		...
		# 随后的 AbilityRemoved 触发 REMOVE,语义和 BuffTranslator 对齐。
		var summary := ShieldSummary.new()
		summary.id = ability_id
		summary.config_id = config_id
		...
		actions.append(VisualShieldStateAction.new(
			actor_id, VisualShieldStateAction.Op.UPDATE, ability_id, summary
		))
