## DamageTranslator - 伤害事件翻译员
##
## 将 damage 事件翻译为飘字、受击闪白和血条更新卡片
## 注意：攻击特效由 StageCueTranslator 处理，不在此处
class_name FrontendDamageTranslator
extends Translator


func _init() -> void:
	translator_name = "DamageTranslator"
...
func translate(event: Dictionary, query: VisualStateQuery) -> Array[VisualAction]:
	...
	if actual_life_damage > 0.0:
		...
		if is_reflected:
			text = "反伤 " + text
		var style := VisualFloatingTextAction.FloatingTextStyle.CRITICAL if is_critical else VisualFloatingTextAction.FloatingTextStyle.NORMAL
	...
	if shield_absorbed > 0.0:
		...
		var absorb_floating := VisualFloatingTextAction.new(
			target_id,
			absorb_text,
			Color(0.4, 0.8, 1.0),  # 浅蓝
			target_position,
			VisualFloatingTextAction.FloatingTextStyle.NORMAL,
			config.damage_floating_text_duration
		)
	...
	# visual_hp 由 VisualUpdater 每 tick lerp 收敛)。全吸收时血条不动。
