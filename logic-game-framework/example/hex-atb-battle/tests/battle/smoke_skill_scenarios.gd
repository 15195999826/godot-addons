...
##
## 扫 `addons/logic-game-framework/example/hex-atb-battle/tests/battle/skill_scenarios/`
## 下除基础设施文件外的所有 *.gd:
##   - 加载脚本
...
const SCENARIOS_DIR := "res://addons/logic-game-framework/example/hex-atb-battle/tests/battle/skill_scenarios/"
...
func _report_and_exit() -> void:
	...
	var reason := "%d/%d scenarios passed" % [pass_count, total]

	# context 只许活在调用栈上：全部 scenario 跑完，两类 context 的存活数必须归零。
	...
	if live_contexts != 0:
		...
		all_pass = false
		reason += "; %d contexts outlived their call stack" % live_contexts

	print("")
	var marker := "PASS" if all_pass else "FAIL"
	print("SMOKE_TEST_RESULT: %s - %s" % [marker, reason])
