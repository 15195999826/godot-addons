...
## - totem actor 在战斗结束前已 remove (replay 含 ability_removed for TotemLifetime)
...
func assert_replay(ctx: ScenarioAssertContext) -> void:
	# 1. 找 spawn 事件: TOTEM 上的 ability_granted 含 TotemAttack 或 TotemLifetime config_id
	#    (TOTEM actor 是新 spawn 的, 没有自己的 ability_granted 通知 actor 本身, 但它接收 grant 自带 passives)
	...
	for e in ctx.events_of_kind(GameEvent.ABILITY_GRANTED_EVENT):
		...
		var cfg_id := str(ability_data.get("config_id", ""))
		if cfg_id == HexBattleTotemAttack.CONFIG_ID:
			totem_attack_grants.append(e)
			totem_actor_id = str(e.get("actor_id", ""))
	...
	for e in ctx.events_of_kind(GameEvent.STAGE_CUE_EVENT):
		if str(e.get("source_actor_id", "")) == totem_actor_id \
				and str(e.get("cue_id", "")) == HexBattleTotemAttack.STAGE_CUE_ID:
			totem_attack_cues.append(e)
	ctx.assert_true(totem_attack_cues.size() >= 1,
		"Expect TotemAttack stage_cue for frontend attack VFX (got %d)" % totem_attack_cues.size())
