...
class_name BasicAttackLandedFireballNoEmitScenario
extends SkillScenario


func get_name() -> String:
	return "BasicAttackLanded: Fireball does NOT emit BasicAttackLandedEvent"
...
func assert_replay(ctx: ScenarioAssertContext) -> void:
	# Fireball 应当命中 (damage event 存在), 但不携带 BasicAttackLandedEvent。
	...
	ctx.assert_eq(attack_events.size(), 0,
		"Fireball must NOT emit any BasicAttackLandedEvent (got %d)" % attack_events.size())
