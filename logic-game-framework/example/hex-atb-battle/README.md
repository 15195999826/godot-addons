...
- **尸体不 tick**：`HexBattleProcedure.tick_once` 只 tick 活着的 character 与活着的 mid-spawn actor（图腾 / 火焰地块），死者的整套 ability runtime 冻结——身上的 DOT 不再结算、周期 buff 不再倒计时、尸体的荆棘不再反弹、死前已开始的技能执行也不再落地；死者响应自己那一下致死一击（荆棘 / 亡语）仍由 `is_event_responsive` 保证。skill-preview procedure 一直如此，主战斗 2026-09-14 补齐，`smoke_random_battle_golden` 钉住。**从死亡那一帧起成立**：主循环的存活名单是开趟时建的，排在击杀者之后、当帧稍早被打死的角色轮到时已是尸体——循环里逐个再判 `is_dead()`，当帧就不再 tick / 充能 / 起手（否则尸体当帧还能出一刀、发一箭、起手一步；skill-preview 的参战者循环同一条合同）。死者起手而未走完的行动（move / skill 的在飞 execution）在它被跳过的那一帧就地 cancel（on_cancel 照常走，Move 的目的地预订已随清足迹清掉），不以「执行中」残留在尸体上；身上 buff / 被动 / 内建能力的周期 execution 随尸体冻结、不 cancel。补 tick 循环遍历 registry 快照：图腾 / 火焰地块寿命到期在自己 tick 里 `remove_actor`，排在它后面的 actor 本帧照常 tick（`smoke_tick_loop_removal` 钉住，skill-preview 的环境物循环同形）。
- **ATB 冻结只认「行动」**：只有角色花行动条换来的那个行动在飞才停充能——主动技能（`active` tag）与移动（`action` tag，Move 专用），判据是 `BattleAbilitySet._is_blocking_execution` 的白名单。buff / 被动 / 内建能力的周期 timeline（中毒 DOT、涌动、恶魔形态、回血）在身期间全程「执行中」，当成阻塞 = 持有者整段不充能不行动（中毒即定身、恶魔形态持有者整场零行动）。白名单而非逐个豁免：新写的周期能力默认不冻结；反方向的漏标（主动技能忘带 `active`、Move 丢了 `action`）由 `smoke_manifest_lint` 断言 4 守，行为由 `smoke_action_tags` 钉住。skill-preview 收尾判 idle 问的是另一个问题（`has_pending_execution`：除内建能力外还有 execution 在飞，DOT 也等它跳完），两张清单不合并。
