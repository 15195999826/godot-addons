...
- **尸体不 tick**：`HexBattleProcedure.tick_once` 只 tick 活着的 character 与活着的 mid-spawn actor（图腾 / 火焰地块），死者的整套 ability runtime 冻结——身上的 DOT 不再结算、周期 buff 不再倒计时、尸体的荆棘不再反弹、死前已开始的技能执行也不再落地；死者响应自己那一下致死一击（荆棘 / 亡语）仍由 `is_event_responsive` 保证。skill-preview procedure 一直如此，主战斗 2026-09-14 补齐，`smoke_random_battle_golden` 钉住。
