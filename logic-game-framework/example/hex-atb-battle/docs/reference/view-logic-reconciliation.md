...
死者的 ability 还会不会被 tick 由 procedure 决定, 与 `HexBattleActor.is_event_responsive(event_dict, phase)` ([hex_battle_actor.gd](../../logic/hex_battle_actor.gd)) 无关——后者只管死者还响应哪些事件 (只剩 post 阶段自己的 death 与自己挨的 damage). 死者身上的 buff 到期被 revoke 时 logic 推 `ABILITY_REMOVED`, view 照单移除; 没到期就仍挂着. 两种情况双方都对称.
