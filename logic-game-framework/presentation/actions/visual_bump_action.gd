## VisualBumpAction - 临时位移弹回卡片
##
## 用于撞墙 / 撞单位时的"冲出 → 卡住 → 弹回"表演。actor 不真的离开当前位置,
## 只是 view 层在位置上叠一段瞬时偏移 + 挤压压扁,逻辑位置完全不动。
##
## 偏移在逻辑平面里描述:direction 是一段逻辑位移(hex 里 = 从停住格指向撞向格的一步 axial 位移,
## 不归一化——axial 平面没有度规,归一化会让不同方向的一步投影后长短不一),max_offset 是这段位移
## 的比例(0.30 = 冲出三成格距)。view 投影是线性映射、保比例,视觉上就是格距 × 0.30。
##
## 时间曲线 (progress 0~1):
...
func _init(
	...
	super._init(KIND_BUMP, p_duration, p_delay)
	actor_id = p_actor_id
