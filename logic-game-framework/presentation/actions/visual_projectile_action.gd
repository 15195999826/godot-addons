## VisualProjectileAction - 投射物飞行卡片
##
## 描述投射物从起点飞向目标的动画
## 支持追踪目标或飞向固定位置
class_name VisualProjectileAction
...
func _init(
	...
	super._init(KIND_PROJECTILE, p_duration, p_delay)
...
func get_current_position(progress: float) -> Vector2:
	return VisualAction.lerp_vector2(start_position, target_position, progress)
