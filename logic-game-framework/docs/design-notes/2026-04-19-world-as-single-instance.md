...
**未实装(按阶段推进)**:`WorldView` / `BattleAnimator`(阶段 2)、`SkillPreviewWorldGI.reset()`(阶段 3)、`ReplayPlayer` + 录像 v3(阶段 4)、`main.tscn` / `Simulation.tscn` 切换到 WorldGI 承载(阶段 5)。
...
2. 阶段 2 起稿：`WorldView`（订阅 WorldGI signal）+ `BattleAnimator`（消费 event_timeline）
