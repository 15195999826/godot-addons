...
### Added
- **`core/ai_decision/` AI 决策管线模块**：「一次决策」的形状——`DecisionSnapshot`（AI 眼中的世界切片，三合同：只读/切片非整世界/感知=构造期过滤策略）、`DecisionOption`（候选：id + payload + requires/provides 供需标签）、`OptionProvider` / `Reasoner`（接口，候选生成与选择算法全归项目——机制与策略分离）、`DecisionResult`（选中项 + reason_key + 回溯链 + 全量打分 breakdown）、`DecisionPipeline.run()`（static 组装，强制两条纪律：候选按 id 字典序排定+重复即断言——运行时偶然序破坏 replay 可重现；非空结果必须带 reason_key——可解释性是合同不是自觉）。附可选基类 `GoalBacktrackReasoner`（GOAP-lite：无谓词世界、无假未来——requires 缺口沿 provides 供需表回溯找准备步且每步对真实世界重验，`max_chain_depth` 默认 2；打分/条件判定/缺口标签/目标排序四个策略孔归子类）。模块无循环、无执行、无时机、不持有 RNG（参数传入，确定性责任在调用方）。测试 `tests/core/ai_decision/decision_pipeline_test.gd`（11 用例：确定性/回溯链/深度限制/环防护/同分兜底/breakdown 约定）。why：多项目（town 生活模拟 AI / battle AI）需要同一套可解释、可回放的决策基础设施；框架只固化「事实→候选→选择→解释」的接口与纪律，不绑架任何单一算法。
